...
export default function AdminPanel({ config: propConfig, onLogout }: AdminPanelProps) {
  ...
  const [expandedVehiclesMap, setExpandedVehiclesMap] = useState<Record<string, string | null>>({});
  ...
  useEffect(() => {
    if (token) {
      // Verificar validez del token con el servidor inmediatamente al cargar
      fetch('/api/verify-token', {
        headers: { 'Authorization': `Bearer ${token}` }
      }).then(async (res) => {
        if (!res.ok && res.status === 401) {
          console.warn('[AdminPanel] Sesión expirada o token inválido en inicio.');
          localStorage.removeItem('mastertech_admin_token');
          setToken(null);
          setAuthError('Tu sesión anterior ha expirado. Por favor ingresa tus credenciales.');
        } else if (res.ok) {
          const renewed = res.headers.get('x-renewed-token');
          if (renewed) {
            localStorage.setItem('mastertech_admin_token', renewed);
            setToken(renewed);
          }
        }
      }).catch(() => {});
  ...
  const handleSaveSection = async (sectionKey: string, customPayload?: any) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          const pass = window.prompt("⚠️ Tu sesión de administrador ha expirado por seguridad.\n\nIngresa tu contraseña para renovar la sesión y guardar tus cambios de inmediato sin perder nada:");
          if (pass && pass.trim()) {
            const loginRes = await fetch('/api/login', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                email: currentUser?.email || 'admin@tallermastertech.com',
                password: pass.trim()
              })
            });
            const loginData = await loginRes.json();
            if (loginRes.ok && loginData.token) {
              localStorage.setItem('mastertech_admin_token', loginData.token);
              if (loginData.user) {
                localStorage.setItem('mastertech_admin_user', JSON.stringify(loginData.user));
                setCurrentUser(loginData.user);
              }
              setToken(loginData.token);
              
              // Reintento inmediato con el nuevo token válido
              ...
              if (retryRes.ok) {
                ...
                const confirmedSettings = { ...(retryData.settings || {}), ...targetForm };
                ...
                alert('✅ ¡Sesión renovada y cambios guardados exitosamente en Supabase!');
                setTimeout(() => setSavedSectionSuccess(null), 4000);
                return;
              }
            } else {
              alert(`❌ Contraseña incorrecta: ${loginData.error || 'Credenciales inválidas'}`);
              return;
            }
          } else {
            localStorage.removeItem('mastertech_admin_token');
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-4">
                          ...
                          {/* 2. LISTA DE VEHÍCULOS ASIGNADOS Y EN COLA (ACORDEÓN DESPLEGABLE) */}
                                        ...
                                        <div className="p-3.5 pt-0 border-t border-white/5 space-y-3 mt-1 animate-fade-in">
                                          ...
                                          <div className="space-y-2">
                                            <div className="flex items-center justify-between gap-2 border-b border-white/5 pb-1">
                                              <span className="text-[10px] font-black uppercase tracking-wider text-amber-400 flex items-center gap-1.5 min-w-0">
                                                ...
                                                <span className="truncate">LISTA DE TRABAJOS / TAREAS MECÁNICAS AUTORIZADAS</span>
                                              </span>
                                          ...
                                          {v.notasInternas && (
                                            <div className="p-2 rounded-xl bg-white/5 border border-white/10 text-[11px] text-zinc-300 leading-relaxed flex items-start gap-2">
                                              ...
                                              <span className="italic">{v.notasInternas}</span>
                                            </div>
                                          )}
